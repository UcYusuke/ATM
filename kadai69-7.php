...
define(
    ...
    array(
        ...
        'FAIL_PASSWORD_THREETIME' => "パスワードを3回間違えたため、終了します",
...
class Atm{
    ...
    private $wrong_count = 0;  //パスワード入力間違いカウント用
    ...
    private function login_user()
    {
        ...
        //ユーザが存在しないとき
        if (User::isExistById($input_id) === false) {
        ...
        $this->user =  [
            'name' => User::findById($input_id), //ユーザIDに紐づくユーザ名を取得
            'pass' => User::USER_LIST[$input_id]['pass'] //パスワード
        ];
    }
    ...
    private function login_pass()
    {
        ...
        if($this->validatePass($input_pass) === false){
            $this->wrong_count++; //間違った回数を+1
            if($this->wrong_count >= 3){
                //3回間違えたら終了
                Atm::outputMessage('FAIL_PASSWORD_THREETIME'); //終了メッセージ
                exit(); 
            }
        ...
        //パスワード一致した場合
        Atm::outputMessage('SUCCESS_LOGIN'); //ログイン成功のメッセージ
