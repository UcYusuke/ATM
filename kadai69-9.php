...
class Validation{
    ...
    /************************************************
     * パスワードチェック
     * @param string $pass 標準入力されたパスワード
     * @param Atm $atm Atmクラスのインスタンス
     * @return bool 標準入力されたパスワードがAtmクラスのuser['pass']と異なる場合false,そうでない場合trueを返す
     ****************************************************/
    public function validatePass($pass, $atm){     
        //ユーザプロパティにセットされたパスワードと一致しない場合
        if ($pass !== $atm->getUser()['pass']) { //★★★★　Atmクラスのインスタンスを渡す表現に修正 ★★★★
            // $this->outputMessage('FAIL_PASSWORD'); //★★★★　削除しAtmクラスへ移動 ★★★★
            return false;
        }
        //パスワードが一致する場合
        return true;
    ...
    function validateMenu($input){
        if (isset(Atm::MENUS[$input]) === false) {
            // $this->outputMessage('ERROR_SELECTMENU'); //★★★★　削除しAtmクラスへ移動 ★★★★
            return false;
        }
        return true;
    }
    ...
    function validateDeposit($input){
        if ($input <= 0) {
            //$this->outputMessage('ERROR_AMOUNT'); //★★★★　削除しAtmクラスへ移動 ★★★★
            return false;
        }
        return true;
    }
     ...
     ****************************************************/
    function validateWithdraw($input, $atm){ 
        if ($input <= 0) {
            //$this->outputMessage('ERROR_AMOUNT'); //★★★★　削除しAtmクラスへ移動 ★★★★
            return false;
        }
        if ($input > $atm->getAccountBalance()) { //★★★★　Atmクラスのインスタンスを引数に渡す表現に修正 ★★★★
            //$this->outputMessage('FAIL_WITHDRAW'); //引き出し失敗のメッセージ //★★★★　削除しAtmクラスへ移動 ★★★★
            return false;
        }
        return true;
    }
...
class Atm
{
    ...
    private function login_user(){
        ...
        if ($is_exist === false) {
            ...
            if ($this->wrong_count_pass >= Atm::MAX_WRONG_COUNT) {
    ...
    private function login_pass(){
        ...
        //Validationクラスのインスタンスを作成
        $validation = new Validation(); // ★★★★　追記　★★★★

        //パスワードが一致しない場合
        if ($validation->validatePass($input_pass, $this) === false) { // ★★★★ validationクラスを用いた表現に修正 ★★★★　
            $this->outputMessage('FAIL_PASSWORD'); //エラーメッセージを表示　//★★★★ 追記 ★★★★
            $this->wrong_count_user++; //間違った回数を+1
            if ($this->wrong_count_user >= Atm::MAX_WRONG_COUNT) {
                $this->exitProcess(); //3回間違えたら終了
            }
  ...
  /************************************************
    ...
    public function selectMenu(){
        ...
        if ($validation->validateMenu($input) === false) { // ★★★★ validationクラスを用いた表現に修正 ★★★★
            $this->outputMessage('ERROR_SELECTMENU'); //メッセージを表示 //★★★★ 追記 ★★★★
    ...
    public function deposit(){
        ...
        if ($validation->validateDeposit($input) === false) { // ★★★★ validationクラスを用いた表現に修正 ★★★★
            $this->outputMessage('ERROR_AMOUNT'); //メッセージを表示 //★★★★ 追記 ★★★★
    ...
    ****************************************************/
    public function withdraw(){
        ...
        if ($validation->validateWithdraw($input, $this) === false) { // ★★★★ validationクラスを用いた表現に修正 ★★★★
            if ($input <= 0) {
                $this->outputMessage('ERROR_AMOUNT'); //★★★★　追記 ★★★★
            }
            if ($input > $this->getAccountBalance()) { 
                $this->outputMessage('FAIL_WITHDRAW'); //引き出し失敗のメッセージ //★★★★　追記 ★★★★
            }
